...
export default async function handler(req, res) {
    ...
    if (error)
        return res.status(error.message.includes('session_full') ? 409 : 500).json({ error: error.message });
    const { data: session } = await supabase
        .from('sessions')
        .select('schools(name)')
        .eq('id', parsed.data.sessionId)
        .single();
    const joinedSchool = session?.schools;
    ...
    return res.status(200).json({ data: { bookingId } });
}
